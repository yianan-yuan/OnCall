...
class OnCallApp {
    constructor() {
        ...
        // AIOps 记进会话时用的用户侧文案，与后端 AIOPS_SESSION_PROMPT 保持一致
        this.aiOpsSessionPrompt = '触发 AI Ops 智能运维排查';
        ...
        // 从后端加载历史会话列表：会话存在 Redis 里，刷新页面/换浏览器都能看到，
        // 不再只依赖浏览器 localStorage
        this.loadSessionsFromServer();

        // 首次进入时同步一次上下文占用（当前会话是全新的，会显示灰色空环）
        this.refreshContextRing();
    }

    // 从后端加载会话列表（后端存在 Redis，这里只拿摘要，消息内容按需再拉）
    async loadSessionsFromServer() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/chat/sessions?limit=50`);
            if (!response.ok) {
                return;
            }
            ...
            // 本地已缓存消息内容的会话保留本地内容，避免每次刷新都重复拉取
            const localById = new Map(this.chatHistories.map(h => [h.id, h]));
            this.chatHistories = result.data.map(item => {
                const local = localById.get(item.sessionId);
                return {
                    id: item.sessionId,
            ...
            this.renderChatHistory();
            console.log('[历史] 已从后端加载会话列表，共', this.chatHistories.length, '条');
        } catch (e) {
            // 后端不可用时仍可用本地历史，不影响聊天
            console.warn('[历史] 从后端加载会话列表失败，继续使用本地历史:', e);
        }
    }
    ...
    initializeElements() {
        ...
        // 上下文占用环与压缩确认弹窗
        this.contextRingBtn = document.getElementById('contextRingBtn');
        this.contextRingValue = document.getElementById('contextRingValue');
        this.contextRingTip = document.getElementById('contextRingTip');
        this.compactModal = document.getElementById('compactModal');
        this.compactModalText = document.getElementById('compactModalText');
        this.compactCancelBtn = document.getElementById('compactCancelBtn');
        this.compactConfirmBtn = document.getElementById('compactConfirmBtn');
    ...
    bindEvents() {
        ...
        // 占用环：点击 = 手动压缩当前会话（先弹窗确认）
        if (this.contextRingBtn) {
            this.contextRingBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                this.openCompactModal();
            });
        }

        if (this.compactCancelBtn) {
            this.compactCancelBtn.addEventListener('click', () => this.closeCompactModal());
        }

        if (this.compactConfirmBtn) {
            this.compactConfirmBtn.addEventListener('click', () => this.confirmCompact());
        }

        // 点遮罩区域也能关掉弹窗
        if (this.compactModal) {
            this.compactModal.addEventListener('click', (e) => {
                if (e.target === this.compactModal) {
                    this.closeCompactModal();
                }
            });
        }
    }
    ...
    // 只取后端记忆状态，不渲染（占用环与压缩弹窗共用）
    async fetchMemory() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/chat/session/${encodeURIComponent(this.sessionId)}/memory`);
            if (!response.ok) {
                return null;
            }
            const result = await response.json();
            ...
            // 后端不可用时环保持上一次状态，不影响聊天
            console.warn('[上下文] 获取记忆状态失败:', e);
            return null;
        }
    }

    // 刷新占用环：后端返回前会按最新消息重算占用，所以答完再拉就是准的
    async refreshContextRing() {
        ...
        // 请求期间可能已经切换会话，丢弃过期结果
        if (!memory || requestedSessionId !== this.sessionId) {
            return;
        }
        this.renderContextRing(memory);
    }

    // 渲染环形进度与悬停提示
    renderContextRing(memory) {
        // 半径 15 的圆周长，用来把占用率换算成 stroke-dashoffset
        const circumference = 2 * Math.PI * 15;
        const exists = memory.exists === true;
        const percent = exists ? Math.min(Math.max(Number(memory.contextUsagePercent) || 0, 0), 100) : 0;

        this.contextRingValue.style.strokeDasharray = `${circumference}`;
        this.contextRingValue.classList.remove('level-normal', 'level-warning', 'level-danger', 'level-empty');

        if (!exists) {
            // 还没聊过的会话：灰色空环，不显示百分比
            this.contextRingValue.classList.add('level-empty');
            this.contextRingValue.style.strokeDashoffset = `${circumference}`;
            this.contextRingTip.innerHTML = '<span class="tip-title">暂无上下文</span><span class="tip-dim">这条会话还没有对话内容</span>';
            return;
        }

        // 三档配色：70% 是自动压缩线，95% 是硬上限
        const level = percent >= 95 ? 'danger' : (percent >= 70 ? 'warning' : 'normal');
        this.contextRingValue.classList.add(`level-${level}`);
        this.contextRingValue.style.strokeDashoffset = `${circumference * (1 - percent / 100)}`;

        const modeNames = {
            'context_70_percent': '占用 70% 自动压缩',
            'every_30_turns': '每 30 轮压缩一次',
            'manual': '仅手动压缩'
        };
        this.contextRingTip.innerHTML = `
            <span class="tip-title">上下文占用 ${percent.toFixed(1)}%</span>
            已用 ${memory.contextTokens || 0} / ${memory.contextWindowTokens || 0} tokens<br>
            消息 ${memory.messageCount || 0} 条，已摘要到第 ${memory.compactedMessageCount || 0} 条<br>
            模式：${modeNames[memory.mode] || memory.mode || '-'}<br>
            存储：${this.escapeHtml(memory.store || '-')}<br>
            <span class="tip-dim">点击可手动压缩</span>
        `;
    }

    // 打开压缩确认弹窗，先把当前占用读出来给用户看
    async openCompactModal() {
        ...
        this.compactModal.classList.add('show');
        this.compactModalText.innerHTML = '正在读取当前占用…';
        this.compactConfirmBtn.disabled = true;

        const memory = await this.fetchMemory();
        if (!memory || memory.exists !== true) {
            this.compactModalText.innerHTML = '这条会话还没有对话内容，暂时不需要压缩。';
            return;
        }

        const percent = Number(memory.contextUsagePercent) || 0;
        this.compactModalText.innerHTML = `
            当前占用 <strong>${percent.toFixed(1)}%</strong>（${memory.contextTokens || 0} / ${memory.contextWindowTokens || 0} tokens）。<br>
            压缩会把滑动窗口之外的较早对话并入摘要，最近几轮原文仍然保留，历史原文一条都不会删。<br>
            <span style="color:#9aa0a6">该操作会调用一次大模型生成摘要，大约需要几秒。</span>
        `;
        this.compactConfirmBtn.disabled = false;
    }

    // 关闭压缩确认弹窗
    closeCompactModal() {
        if (this.compactModal) {
            this.compactModal.classList.remove('show');
        }
    }

    // 确认压缩：调后端压缩接口，用返回的最新状态刷新环与提示
    async confirmCompact() {
        const originalText = this.compactConfirmBtn.textContent;
        this.compactConfirmBtn.disabled = true;
        this.compactConfirmBtn.textContent = '压缩中…';

        try {
            const before = await this.fetchMemory();
            const response = await fetch(`${this.apiBaseUrl}/chat/session/${encodeURIComponent(this.sessionId)}/compact`, {
                method: 'POST'
            });
            const result = await response.json();
            ...
            this.renderContextRing(result.data);

            // 水位线没动说明还没超出滑动窗口，没有可压缩的内容
            const compactedBefore = before ? (before.compactedMessageCount || 0) : 0;
            const compactedAfter = result.data.compactedMessageCount || 0;
            if (compactedAfter > compactedBefore) {
                this.showNotification(`已把 ${compactedAfter - compactedBefore} 条消息并入摘要`, 'success');
            } else {
                this.showNotification('没有可压缩的内容（对话还没超出滑动窗口）', 'info');
            }
            this.closeCompactModal();
        } catch (e) {
            this.showNotification('压缩失败: ' + e.message, 'error');
        } finally {
            this.compactConfirmBtn.disabled = false;
            this.compactConfirmBtn.textContent = originalText;
        }
    }
    ...
    // 加载历史对话（消息内容优先用本地缓存的；本地没有则从后端拉取）
    async loadChatHistory(historyId) {
        ...
        // 本地没有消息内容时（典型场景：刚刷新页面，列表来自后端），从后端取完整消息
        if ((!history.messages || history.messages.length === 0) && history.messageCount !== 0) {
            try {
                const response = await fetch(`${this.apiBaseUrl}/chat/session/${historyId}/messages`);
                const result = await response.json();
                if (result.code === 200 && Array.isArray(result.data)) {
                    history.messages = result.data.map(item => ({
    ...
    deleteChatHistory(historyId) {
        ...
        // 同时清理后端（Redis）里的会话：
        // 否则刷新页面后 loadSessionsFromServer() 会把它又加载回来，看起来像"删不掉"
        fetch(`${this.apiBaseUrl}/chat/clear`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ Id: historyId })
        }).catch(err => console.warn('[历史] 清理后端会话失败:', err));
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '问问小智助手';
        }
    }
    ...
    handleStreamComplete(assistantMessageElement, fullResponse) {
        ...
        if (fullResponse) {
            ...
            if (this.isCurrentChatFromHistory) {
                // 当前对话是从历史记录加载的：更新原有记录
                this.updateCurrentChatHistory();
            } else {
                // 当前对话是新对话：保存为新的历史记录。
                // ⚠️ 这里必须调用 saveCurrentChat()，否则刷新页面后这段对话会丢失——
                //    此前只把消息 push 进了内存数组，从未写入 localStorage，
                //    导致"新对话回答完 → 刷新页面 → 侧边栏没有历史"。
                //    saveCurrentChat() 内部按 sessionId 判重，重复调用只会更新不会重复新增。
                this.saveCurrentChat();
            }
            this.renderChatHistory();
        }
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            // 带上会话 ID：后端据此把报告写回这条会话（与后端 AIOPS_SESSION_PROMPT 文案保持一致）
            const response = await fetch(`${this.apiBaseUrl}/ai_ops?sessionId=${encodeURIComponent(this.sessionId)}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            let fullResponse = '';
            const details = [];   // 执行步骤明细，"查看详细步骤"面板用
            ...
            // 统一处理一条 SSE 消息：content 累积正文、detail 累积步骤、done/error 结束
            const handleSseMessage = (sseMessage) => {
                ...
                } else if (sseMessage.type === 'detail') {
                    details.push(sseMessage.data || '');
                    if (loadingMessageElement) {
                        // 边跑边刷新步骤面板，不然要等几十秒才看到过程
                        this.renderAIOpsDetails(loadingMessageElement, details);
                    }
                } else if (sseMessage.type === 'done') {
                    console.log('AI Ops 流完成，正文长度:', fullResponse.length, '步骤数:', details.length);
                ...
                while (true) {
                    ...
                    for (const line of lines) {
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            // 解析可能包含多个JSON对象的数据
                            // 注意：步骤明细里带引号（工具入参/结果），下面这个正则匹配不到，会走单对象 JSON.parse 分支
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            if (handleSseMessage(JSON.parse(jsonStr)) === 'done') {
                                                return true;
                                            }
                                        } catch (e) {
                            ...
                            if (result === true) {
                                return { fullResponse, details }; // 流结束
                            } else if (result === null) {
                                // 没有匹配到多个JSON，尝试单个JSON解析
                                try {
                                    const sseMessage = JSON.parse(rawData);
                                    if (sseMessage && sseMessage.type) {
                                        if (handleSseMessage(sseMessage) === 'done') {
                                            return { fullResponse, details };
                                        }
                                    } else {
                                        fullResponse += rawData;
                                        if (loadingMessageElement) {
    ...
    // 渲染（或刷新）"查看详细步骤"折叠面板
    // 流式过程中每个步骤到达都会调一次，所以要能重复执行并保留用户已展开的状态
    renderAIOpsDetails(messageElement, details) {
        if (!messageElement || !details || details.length === 0) {
            return;
        }
        ...
        const existingContent = detailsContainer ? detailsContainer.querySelector('.details-content') : null;
        const wasExpanded = existingContent ? existingContent.classList.contains('expanded') : false;
    ...
    updateAIOpsMessage(messageElement, response, details) {
        ...
        // 详情部分（可折叠）- 先显示
        this.renderAIOpsDetails(messageElement, details);
    ...
    async triggerAIOps() {
        ...
        // 不再另开会话：报告直接追加到当前会话，之后可以接着追问；刷新后也能从"近期对话"点回来
        this.addMessage('user', this.aiOpsSessionPrompt);
        ...
        let fullResponse = '';
        try {
            const result = await this.sendAIOpsRequest(loadingMessage);
            ...
            // 后端已把报告写入会话，本地历史补上同一份内容，两边保持一致
            const report = this.extractAiOpsReport(fullResponse);
            if (report) {
                this.currentChatHistory.push({
                    type: 'assistant',
                    content: report,
            ...
            // 报告会明显推高占用，立刻刷新环让用户看到
            this.refreshContextRing();
        }
    }

    // 从 AIOps 的流式文本里取出报告正文
    // 后端只把 Planner 的最终报告写入会话，这里也丢掉"正在读取告警…"这类过程提示，保证两边存的是同一份
    extractAiOpsReport(fullResponse) {
        if (!fullResponse) {
            return '';
        }
        const marker = '📋 **告警分析报告**';
        const markerIndex = fullResponse.indexOf(marker);
        const body = markerIndex >= 0 ? fullResponse.substring(markerIndex + marker.length) : fullResponse;
        // 去掉报告首尾的等号分隔线
