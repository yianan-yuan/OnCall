...
public class AiOpsService {
     ...
     * 默认的 AIOps 排查任务提示词
     * <p>
     * 手动触发（/api/ai_ops）时使用；告警自动触发时会传入带具体告警信息的提示词。
     */
    private static final String DEFAULT_AI_OPS_TASK_PROMPT =
            "你是企业级 SRE，接到了自动化告警排查任务。请结合工具调用，执行**规划→执行→再规划**的闭环，"
                    + "并最终按照固定模板输出《告警分析报告》。禁止编造虚假数据，"
                    + "如连续多次查询失败需诚实反馈无法完成的原因。";

    /**
     * 执行 AI Ops 告警分析流程（使用默认任务提示词）
     *
    ...
    public Optional<OverAllState> executeAiOpsAnalysis(ChatModel chatModel, ToolCallback[] toolCallbacks) throws GraphRunnerException {
        return executeAiOpsAnalysis(chatModel, toolCallbacks, DEFAULT_AI_OPS_TASK_PROMPT);
    }

    /**
     * 执行 AI Ops 告警分析流程（可自定义任务提示词）
     * <p>
     * 之所以开放任务提示词：告警自动诊断需要把"具体是哪条告警、哪个服务、什么描述"带进编排，
     * 否则 Planner 只能面对一个泛泛的排查任务，无法针对性检索知识库与查日志。
     *
     * @param chatModel      大模型实例
     * @param toolCallbacks  工具回调数组
     * @param taskPrompt     任务提示词，为空时使用默认提示词
     * @return 分析结果状态
     * @throws GraphRunnerException 如果 Agent 执行失败
     */
    public Optional<OverAllState> executeAiOpsAnalysis(ChatModel chatModel,
                                                       ToolCallback[] toolCallbacks,
                                                       String taskPrompt) throws GraphRunnerException {
        logger.info("开始执行 AI Ops 多 Agent 协作流程");
        return buildSupervisorAgent(chatModel, toolCallbacks).invoke(effectiveTaskPrompt(taskPrompt));
    }

    /**
     * 以流式方式执行 AI Ops 流程
     * <p>
     * 与阻塞版共用同一套 Agent 编排，区别是返回节点输出流，调用方可以边跑边把增量文本与工具步骤推给前端；
     * 告警自动诊断不需要过程展示，仍用阻塞版。
     *
     * @param chatModel     大模型实例
     * @param toolCallbacks 工具回调数组
     * @param taskPrompt    任务提示词，为空时使用默认提示词
     * @return 节点输出流，需由调用方消费
     * @throws GraphRunnerException 构图失败时抛出
     */
    public Flux<NodeOutput> streamAiOpsAnalysis(ChatModel chatModel,
                                                ToolCallback[] toolCallbacks,
                                                String taskPrompt) throws GraphRunnerException {
        logger.info("开始执行 AI Ops 多 Agent 协作流程（流式）");
        return buildSupervisorAgent(chatModel, toolCallbacks).stream(effectiveTaskPrompt(taskPrompt));
    }

    /**
     * 构建 Supervisor Agent 及其 Planner / Executor 子 Agent
     */
     ...
     * 任务提示词为空时回退到默认提示词
     *
     * @param taskPrompt 调用方传入的提示词
     * @return 实际使用的提示词
     */
    private String effectiveTaskPrompt(String taskPrompt) {
        return (taskPrompt == null || taskPrompt.isBlank()) ? DEFAULT_AI_OPS_TASK_PROMPT : taskPrompt;
    }
