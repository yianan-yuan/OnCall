...
# 默认目标：显示帮助信息
help:
	@echo "$(GREEN)OnCall Makefile$(NC)"
	@echo ""
	@echo "可用命令："
	@echo "  $(YELLOW)make init$(NC)    - 🚀 一键初始化（启动Docker → 启动服务 → 上传文档）"
	@echo "  $(YELLOW)make up$(NC)      - 启动全部中间件（Milvus / Redis / Prometheus / Alertmanager）"
	@echo "  $(YELLOW)make down$(NC)    - 停止全部中间件（保留数据）"
